...
• h = hours (1h = 1 hour)"""
    ...
    # ===== Set Primary API =====
    @app.on_callback_query(filters.regex("^admin_set_primary$"))
    async def admin_set_primary_callback(client, callback_query):
        if not is_admin(callback_query.from_user.id):
            await callback_query.answer("❌ Admin access required", show_alert=True)
            return
        ...
        try:
            await callback_query.answer()
            primary = db.get_primary_api()
            secondary = db.get_secondary_api()
            
            buttons = InlineKeyboardMarkup([
                [InlineKeyboardButton("TeraAPI" if primary != "teraapi" else "✅ TeraAPI", callback_data="api_primary_teraapi")],
                [InlineKeyboardButton("iTeraPlay" if primary != "iteraplay" else "✅ iTeraPlay", callback_data="api_primary_iteraplay")],
                [InlineKeyboardButton("← Back", callback_data="admin_settings")]
            ])
            
            text = Script.API_SETTINGS_TEXT.format(primary=primary, secondary=secondary)
            await client.send_message(callback_query.message.chat.id, text, reply_markup=buttons, parse_mode=enums.ParseMode.HTML)
            await callback_query.message.delete()
        except Exception:
            log.exception("admin_set_primary_callback error")
    
    # ===== Set Secondary API =====
    @app.on_callback_query(filters.regex("^admin_set_secondary$"))
    async def admin_set_secondary_callback(client, callback_query):
        if not is_admin(callback_query.from_user.id):
            await callback_query.answer("❌ Admin access required", show_alert=True)
            return
        ...
        try:
            ...
            buttons = InlineKeyboardMarkup([
                [InlineKeyboardButton("TeraAPI" if secondary != "teraapi" else "✅ TeraAPI", callback_data="api_secondary_teraapi")],
                [InlineKeyboardButton("iTeraPlay" if secondary != "iteraplay" else "✅ iTeraPlay", callback_data="api_secondary_iteraplay")],
                [InlineKeyboardButton("← Back", callback_data="admin_settings")]
            ])
            ...
            log.exception("admin_set_secondary_callback error")
    
    # ===== Primary API Selection Callbacks =====
    @app.on_callback_query(filters.regex("^api_primary_(teraapi|iteraplay)$"))
    async def api_primary_select_callback(client, callback_query):
        if not is_admin(callback_query.from_user.id):
            await callback_query.answer("❌ Admin access required", show_alert=True)
            return
        
        try:
            api_name = callback_query.data.split("_")[2]
            db.set_primary_api(api_name)
            await callback_query.answer(f"✅ Primary API set to {api_name}", show_alert=True)
            await callback_query.message.delete()
            log.info(f"Admin {callback_query.from_user.id} set primary API to {api_name}")
        except Exception:
            log.exception("api_primary_select_callback error")
    
    # ===== Secondary API Selection Callbacks =====
    @app.on_callback_query(filters.regex("^api_secondary_(teraapi|iteraplay)$"))
    async def api_secondary_select_callback(client, callback_query):
        if not is_admin(callback_query.from_user.id):
            await callback_query.answer("❌ Admin access required", show_alert=True)
            return
        ...
        try:
            api_name = callback_query.data.split("_")[2]
            db.set_secondary_api(api_name)
            await callback_query.answer(f"✅ Secondary API set to {api_name}", show_alert=True)
            await callback_query.message.delete()
            log.info(f"Admin {callback_query.from_user.id} set secondary API to {api_name}")
        except Exception:
            log.exception("api_secondary_select_callback error")
